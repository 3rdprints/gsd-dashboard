...
A Tauri 2 cross-platform desktop dashboard that aggregates Get Shit Done project data and Claude Code / Codex session telemetry. macOS primary; Windows and Linux supported. Menu-bar / system-tray presence with live milestone progress bars (openusage-inspired).
...
- `sessions/{claude,codex,matcher,mod}.rs` — JSONL streaming with byte-offset incremental parsing + attribution
...
4. **JSONL partial writes on live files** — Claude Code appends continuously; last line often mid-record. Track `(path, last_parsed_byte_offset)`; tolerate final-line parse error without marking file corrupt.
5. **Claude/Codex session formats are unstable** — community-reverse-engineered schemas; Claude Code auto-updates have deleted `.jsonl` in the past. Multi-version fixtures; `Option<T>` on every field.
