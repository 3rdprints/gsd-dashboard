...
- [ ] **SCAN-02**: User can add or remove scan-root directories in Settings; changes take effect without restart
- [ ] **SCAN-03**: User can hide and unhide individual projects; hidden projects remain discoverable but disappear from the portfolio and tray
- [ ] **SCAN-04**: "Rebuild cache" action in Settings drops the derived cache and re-runs a full scan
...
- [ ] **PORT-01**: Landing view displays one card per non-hidden project, sorted by last activity descending
...
- [ ] **PORT-03**: Clicking a card opens the Project Detail view for that project
...
- [ ] **PORT-06**: Right rail shows collapsed "Hidden projects" and "Unmatched sessions" sections
- [ ] **PORT-07**: Header stats show total projects tracked, active milestones, sessions today, and tokens today

### Project Detail View

- [ ] **DET-01**: Detail view shows project name, root path, and action buttons: "Open in Finder", "Open in VS Code", "Copy next command"
...
- [ ] **CLIP-01**: "Copy next command" writes the project-recommended command (or `/gsd-next` default) to the OS clipboard
- [ ] **CLIP-02**: The dashboard never executes commands directly and never writes into `.planning/` directories
...
- [ ] **SET-01**: Scan-roots editor lets users add/remove directories
- [ ] **SET-02**: Hidden-projects list shows every hidden project with an Unhide action
- [ ] **SET-03**: Tray display preferences: max bars, sort order, per-project show/hide independent of overall hidden state
- [ ] **SET-04**: "Rebuild cache" action available; confirms destructive operation
- [ ] **SET-05**: Phase 2 and Phase 3 toggles exist as placeholders (index tool usage / index message content) — visible but disabled
